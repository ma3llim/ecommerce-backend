...
public interface ProductRepository extends JpaRepository<Product, UUID> {
    ...
    Page<Product> findByIdInAndPublishedTrue(List<UUID> productIds, Pageable pageable);
    ...
    @Query("""
                SELECT new org.ecommerce.catelog.dtos.publics.ProductSearchResponse(p.id, p.name, p.slug) FROM Product p
                WHERE p.published = true AND LOWER(p.name) LIKE LOWER(CONCAT('%', :keyword, '%'))
            """)
